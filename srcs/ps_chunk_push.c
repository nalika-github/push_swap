...
static void ps_pushlogic(t_list **stk_a, t_list **stk_b)
{
	int	top_a;
	int	maxlo;
	int	minlo;

	if (ft_lstsize(*stk_b) < 3)
	{
		ps_dopb(stk_a, stk_b);
		return ;
	}
	top_a = ((t_data *)((*stk_a)->data))->num;
	if(ps_ismax(*stk_b, top_a) || ps_ismin(*stk_b, top_a))
	{
		ps_doedge(stk_b);
		ps_dopb(stk_a, stk_b);
	}
